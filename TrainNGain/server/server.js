  ...
  if (err) {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS balances (
      ...
      if (err) {
        ...
        const initData = () => {
          const stmt = db.prepare("INSERT OR IGNORE INTO balances (id, balance, email, username, bio) VALUES (?, ?, ?, ?, ?)");
          stmt.run('user1', 100, 'user1@gmail.com', 'user1', 'Hi there! I love building apps with React Native and exploring new technologies.');
          stmt.run('user2', 200, 'user2@gmail.com', 'user2', 'Hello there! I love building apps with React Native and exploring new technologies.');
          stmt.finalize(() => {
